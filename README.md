...
一个 HTML5 双人联机的宫斗回合制小游戏，融合五子棋/石头剪刀布/猜大小三种"棋决胜负"决斗，配以道具卡牌与角色职业系统。手机浏览器即可游玩，无需安装。

- **前端**：单页 HTML + 原生 CSS/JS（古风 UI，emoji 立绘 + Canvas 棋盘，移动端优化）
- **后端**：Node.js + Express + Socket.IO
- **联机**：5 位房间号匹配，房主创建后把房号发给朋友即可
- **两种模式**：
  - **宫斗对决**：每月双方暗中选动作，同步揭晓；先晋皇后或多月内综合分高者胜
  - **五子棋**：黑先白后，先连五子者胜，每步限时
- **特殊机制 · 棋决胜负**：宫斗模式中，谁选了「设陷害」就**随机**触发三种决斗（五子棋 / 石头剪刀布 / 猜大小），胜者陷害成功
- **角色职业**：6 种身份（才女 / 妖姬 / 心机 / 嫡女 / 神医 / 良家），各有起始属性与技能加成
- **道具卡牌**：8 张基础卡（鸩茶 / 玉佩 / 蛊毒 / 龟甲符 / 红绒花 / 太后赏赐 / 风铃令 / 密信），月初有概率抽得
- **管理后台**：`/admin.html` 可修改所有参数（决斗池、卡牌掉率、晋升门槛、子嗣名等）
...
- 🎲 **三种决斗任你打**：陷害动作随机触发**五子棋 / 石头剪刀布 / 猜大小**之一
  - 石头剪刀布：三局两胜，同步揭晓
  - 猜大小：暗选 1-100，三局两胜大者赢，提供快捷数字按钮
  - 五子棋：决斗时**棋盘铺满全屏**，触屏点击精准落子
- 👤 **角色职业**：6 种身份（默认 / 才女 / 妖姬 / 心机 / 嫡女 / 神医），起始属性 + 月效应 + 行动加成各异
- 🎴 **道具卡牌**：8 张基础卡，月初有概率抽得，每月限用一张
  - 主动卡：鸩茶（削对方圣宠）/ 红绒花（+美貌）/ 太后赏赐（+圣宠）/ 风铃令（+势力）/ 蛊毒（下月削对方体力）/ 密信（侦察对手）/ 龟甲符（防一次负面事件）
  - 被动卡：玉佩（自动免疫一次陷害）
- 📱 **移动端深度优化**：按键放大（最小 44px 触摸目标）、五子棋全屏决斗、动作按钮 emoji 26px、决斗按键 120px 高
- 🛠️ **管理后台 `/admin.html`**：用 Token 登录后修改所有游戏参数（皇帝名、朝代、月份、晋升门槛、卡牌掉率、决斗池等）
...
├── server.js              # 网络层：Socket.IO + Express + 决斗调度
├── game.js                # 宫斗逻辑（卡牌 + 职业 + 月制资源）
├── gomoku.js              # 五子棋纯逻辑（无网络依赖）
├── config.js              # 运行时配置加载/保存
├── duels/
│   ├── index.js           # 决斗池调度器
│   ├── gomoku.js          # 五子棋决斗适配器
│   ├── rps.js             # 石头剪刀布
│   └── guess.js           # 猜大小
├── public/
│   ├── index.html         # 游戏前端（古风 UI + 三种决斗 UI）
│   └── admin.html         # 管理后台
├── test-game.js           # 宫斗逻辑单测（33000+ 断言）
├── test-gomoku.js         # 五子棋单测
├── test-duels.js          # 三种决斗模块单测
├── test-cards-classes.js  # 卡牌+职业单测
├── test-network.js        # 网络层端到端测试
└── test-integration.js    # v3.1 完整集成测试
```
